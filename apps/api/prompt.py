...
async def get_list(params: dict = None): 
    ...
    session = new_session() 
    prompt_list = session.query(Prompt.prompt_id, Prompt.act, Prompt.prompt).filter(and_(Prompt.category == category),and_(Prompt.is_deleted == 0)).order_by(asc(Prompt.id)).all() 
...
async def delete(user: CurrentUser = Depends(CurrentUser), 
    ...
    session = new_session() 
    try: 
        session.query(Prompt).filter(Prompt.id.in_(id_list)).filter(Prompt.is_deleted == 0).update({"is_deleted": 1, "modifier": user.username, 
...
async def update(user: CurrentUser = Depends(CurrentUser), 
    ...
    session = new_session() 
    try: 
        session.query(Prompt).filter(Prompt.id == id_key).filter(Prompt.is_deleted == 0).update({"category": category, "act": act, "prompt": prompt, 
                     "gmt_modified": datetime.datetime.now()}, synchronize_session=False) 
    except Exception as e: 
        print(e) 
        session.rollback() 
