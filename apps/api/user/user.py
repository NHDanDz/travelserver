...
def find_modify_pwd(request: Request, 
    ...
    session = new_session() 
    try: 
        ret = session.query(Users)  .filter(or_(Users.username == (username or '$$$$$$'), 
                        Users.email == (email or '$$$$$$'))).update({"passwd": md5(new_pwd), "gmt_modified": datetime.now()}, synchronize_session=False) 
        session.commit() 
        if ret: 
